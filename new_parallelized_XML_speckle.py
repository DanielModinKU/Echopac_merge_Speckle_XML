...
def parse_xml(path):
	...
	if len(parameters) >= 1:
		...
		percent_progress = int((xml_count*cpu_count)/n_xml_files*100)
		#print(xml_count)
		if (percent_progress%5 == 0):
			print('Completed (approx): '+str(percent_progress)+'%')
...
def click():  
	...
	####### running data collection in parallel using all cpu cores!
	####### running data collection in parallel using all cpu cores!
	...
	pool = mp.Pool()
	#run pool
	result = pool.map(excel_process,files)

	#cooncatenate into single data
	print('Merging individual data segments')
	df = pd.concat(result)

	end = time.time() #time stamp 
	diff = end - start 

	print('Took this long to process and merge data : '+str(int(diff))+' seconds')

	##Skriv til sti som filerne ligger i
	path_write=path+'/merged_XL.xlsx'
	global xl_path 
	xl_path = path_write
	print('Writing path: '+path_write)
...
def click2():
	...
	pool = mp.Pool()
	#run pool
	result = pool.map(parse_xml,files)
